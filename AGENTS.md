...
- **2026 milestones (planning):** ~**mid-June** — Mac mini always-on host; ~**mid-August** — tiiny.ai heavy-reasoning tier — [docs/V1_1_TO_V2_EXECUTION_TIMELINE.md](docs/V1_1_TO_V2_EXECUTION_TIMELINE.md). Before mid-June, prefer **truthful Deployment/capabilities + stable delegation** over polishing Hermes/OpenClaw prompt copy.
...
- **Delegation (Hermes first, OpenClaw second):** On **`virgil-manos`**, prefer the **Hermes** HTTP bridge (`HERMES_*`) for **`delegateTask`** / **`embedViaDelegation`**; add **OpenClaw** (`OPENCLAW_*`) when you want breadth or a compatibility path. When **both** are configured, Virgil defaults to **Hermes** and can **fail over** to OpenClaw if Hermes is unreachable (`VIRGIL_DELEGATION_FAILOVER`; on by default for dual-stack). **Vercel production (recommended):** use **database poll** — set `VIRGIL_DELEGATION_POLL_PRIMARY=1` and a worker secret (`VIRGIL_DELEGATION_WORKER_SECRET`, or reuse `HERMES_SHARED_SECRET`); Hermes/Manos **polls** `GET /api/delegation/worker/claim` and posts results to `POST /api/delegation/worker/complete` with outbound HTTPS only (no inbound tunnel to your LAN). **Alternative:** expose **Hermes** with a tunnel (e.g. Cloudflare to `manos:8765`) so Virgil can call Hermes over HTTP — keep **`OPENCLAW_*` unset** on Vercel and let Hermes call OpenClaw on `127.0.0.1` on `manos` via a skill. See [docs/virgil-manos-delegation.md](docs/virgil-manos-delegation.md). Bridge contract and env table: [docs/openclaw-bridge.md](docs/openclaw-bridge.md); SSH tunnels: [docs/openclaw-ssh-tunnel-hardening.md](docs/openclaw-ssh-tunnel-hardening.md). Neither gateway is the primary chat LLM. **Routing:** there is **no per-message backend selector** in Virgil — intents go to the primary gateway when it responds to health checks; the secondary is used only when failover is on and the primary is down. **Product decision (dual backends):** there is **no per-request pin** of Hermes vs OpenClaw — only **env-defined primary** plus optional **failover** (`delegationSendIntent` in `lib/integrations/delegation-provider.ts`). Do not add UX or prompts that imply per-call gateway choice. The signed-in **Deployment** page (`/deployment`) summarizes reachability and gateway skill ids (from `GET /api/deployment/capabilities`).
- **Delegation access / limits:** The signed-in **Deployment** page (`/deployment`) and delegation chat tools use the **same session and owner scoping** as the rest of chat (single-tenant posture). Tool allow/deny, rate limits, and OpenClaw-side policy run on the **gateway host**. Virgil is a passthrough plus optional **`VIRGIL_DELEGATION_STRICT_SKILLS`**. Set **`VIRGIL_DELEGATION_TOOLS_DISABLED=1`** to hide **delegateTask** / approvals / **embedViaDelegation** in chat without unsetting bridge env; the Deployment page can still probe reachability.
...
## May 2026 — Vercel Virgil first

Through **May 2026**, **prioritize the Vercel-deployed Virgil path** in planning and implementation: deploy story, environment and CI parity, and product behavior that is correct on that stack by default. **Intent:** lay **groundwork for ubiquity** — keep external boundaries stable (HTTP routes, auth, agent/tool contracts) so future clients and surfaces reuse those contracts instead of growing parallel one-off infra.
...
- **In-app (any environment):** Signed-in users can open **This deployment** from the user menu (`/deployment`) for a **user-safe** summary of hosted vs local inference and companion tools on **this** instance. It complements (does not replace) `pnpm virgil:status` / `GET /api/virgil/status`, which remain oriented toward developer diagnostics.
...
3. **Off-peak scheduling (default 11pm–7am local):** Defaults target **US Eastern** — **`NIGHT_REVIEW_TIMEZONE`** defaults to **`America/New_York`** (handles **EST/EDT**; not a fixed UTC offset). Token-heavy night review only runs inside **`NIGHT_REVIEW_OFF_PEAK_START_HOUR`**–**`NIGHT_REVIEW_OFF_PEAK_END_HOUR`** (default **23**–**7**, end exclusive) when the local clock hour equals **`NIGHT_REVIEW_RUN_LOCAL_HOUR`** (default **23** = **11:00pm**). Reuse **`isNowWithinOffPeakLocal`** / **`isHourWithinOffPeak`** from `lib/night-review/off-peak.ts` for other deferred jobs.
...
5. **Cron:** On **Vercel** (see [`vercel.json`](vercel.json)), **`GET /api/night-review/enqueue`** runs **once daily** at **`5 4 * * *`** (**04:05 UTC**) so **Hobby** plans stay valid. That lines up with **11:05pm Eastern** during **standard time** (late fall–early spring): **04:05 UTC** ≈ **23:05 `America/New_York`**. During **daylight time**, switch the schedule to **`5 3 * * *`** (**03:05 UTC**) for the same **11:05pm** local wall time, or use **self-hosted** **`curl` hourly** so every local hour is sampled (see host cron section below). Vercel cron is always **UTC**; a single expression cannot track **DST** perfectly year-round.
...
|-----|------|----------------|
| Night review enqueue | `/api/night-review/enqueue` | Daily (`5 4 * * *` UTC default; see §1.9 for EDT vs EST) |
...
| `NIGHT_REVIEW_TIMEZONE` | No             | No                 | IANA tz for idempotency `windowKey` and off-peak local wall clock (default `America/New_York`) |
...
| `NIGHT_REVIEW_RUN_LOCAL_HOUR` | No | No | Local hour 0–23 when the scheduled enqueue should run (default `23` = 11pm) |
...
| `VIRGIL_DELEGATION_POLL_PRIMARY` | No | No | Set `1` / `true` so `delegateTask` releases intents to Postgres for a **local poll worker** (`GET/POST /api/delegation/worker/*`) instead of synchronous HTTP from Vercel. Requires `VIRGIL_DELEGATION_WORKER_SECRET` or `HERMES_SHARED_SECRET`. See [docs/virgil-manos-delegation.md](docs/virgil-manos-delegation.md). |
| `VIRGIL_DELEGATION_WORKER_SECRET` | With poll primary | Same | Bearer token for delegation worker routes. If unset, `HERMES_SHARED_SECRET` is used when present. |
| `VIRGIL_DELEGATION_POLL_WAIT_MS` | No | No | Optional: block the chat tool until the poll worker completes (milliseconds, capped at 60000). `0` or unset = immediate “queued” success. |
| `VIRGIL_DELEGATION_WORKER_BASE_URL` | No | No | **Poll worker machine only:** hosted Virgil origin (`https://…`). Same role as `NEXT_PUBLIC_APP_URL` if you already set that for the worker’s `.env.local`. |
| `VIRGIL_DELEGATION_POLL_INTERVAL_MS` | No | No | **Poll worker only:** idle sleep after empty `claim` (default 5000 ms; clamped 1000–120000). |
| `VIRGIL_DELEGATION_WORKER_EXECUTE_TIMEOUT_MS` | No | No | **Poll worker only:** HTTP timeout for each local Hermes `execute` call (default 120000 ms). |
...
| `VIRGIL_DELEGATION_TOOLS_DISABLED` | No | No | Set `1` / `true` / `yes` to hide **delegateTask** / **approveDelegationIntent** / **embedViaDelegation** from chat while Hermes/OpenClaw env remains set. Deployment still probes the bridge; the poll worker may drain queued intents. |
| `OPENCLAW_URL` | No | No | Optional OpenClaw gateway. Hardened default is a local tunnel (`ws://127.0.0.1:13100`) on the Mac: set `OPENCLAW_SSH_HOST` and run `pnpm openclaw:tunnel` (see [docs/openclaw-ssh-tunnel-hardening.md](docs/openclaw-ssh-tunnel-hardening.md) for SSH target and owner reference host). Bridge behavior: [docs/openclaw-bridge.md](docs/openclaw-bridge.md). **On Vercel:** usually **unset** — tunnel Hermes only and proxy OpenClaw from Hermes on `manos` ([docs/virgil-manos-delegation.md](docs/virgil-manos-delegation.md)). |
| `OPENCLAW_HTTP_URL` | No | No | Explicit HTTP origin for OpenClaw REST (defaults from `OPENCLAW_URL`). Hardened tunnel value: `http://127.0.0.1:13100`. **On Vercel:** usually **unset** (same as `OPENCLAW_URL`). |
