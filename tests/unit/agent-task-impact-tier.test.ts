...
test("infra + low → elevated", () => {
  assert.equal(
    tier({ taskType: "infra", priority: "low" }),
    "elevated"
  );
});
...
test("docs + critical → elevated", () => {
  assert.equal(
    tier({ taskType: "docs", priority: "critical" }),
    "elevated"
  );
});
...
test("prompt + medium → standard", () => {
  assert.equal(
    tier({ taskType: "prompt", priority: "medium" }),
    "standard"
  );
});
...
test("feature + high → elevated; feature + medium → standard", () => {
  assert.equal(
    tier({ taskType: "feature", priority: "high" }),
    "elevated"
  );
  assert.equal(
    tier({ taskType: "feature", priority: "medium" }),
    "standard"
  );
});
